...
C
     ...
     *              ,MW2,MZ2,MH2,ME2,MMY2,MTAU2,MU2,MD2,MS2,MC2,MB2,MT2
      ...
      common/couplings/cau,cvu,cad,cvd
...
************************************************************************
      ...
      subroutine EPRC_INIT_ALPOS_FULL(LP1,LP2,LP3,LP4,LP5,
     $     LP6,LP7,LP8,LP9,LP10,
     $     LP11,LP12,LP13,LP14,LP15,
     $     LP16,LP17,LP18,LP19,LP20,
     $     me_i,mmu_i,mtau_i,
     $     mu_i,md_i,ms_i,mc_i,mb_i,mt_i,
     $     mZ_i,mW_i,mH_i,
     $     alpha_i, gf_i,
     $     cau_i,cvu_i,cad_i,cvd_i,
     $     doprint)

      IMPLICIT REAL*8(A-H,M,O-Z)
      logical doprint

      call EPRC_INIT_ALPOS(LP1,LP2,LP3,LP4,LP5,
     $     LP6,LP7,LP8,LP9,LP10,
     $     LP11,LP12,LP13,LP14,LP15,
     $     LP16,LP17,LP18,LP19,LP20,
     $     doprint)
      CALL SETPAR_ALPOS(me_i,mmu_i,mtau_i,
     $     mu_i,md_i,ms_i,mc_i,mb_i,mt_i,
     $     mZ_i,mW_i,mH_i,alpha_i,gf_i,
     $     cau_i,cvu_i,cad_i,cvd_i,doprint)

      if (doprint) write(6,*) 'end of EPRC_INIT_ALPOS_FULL dr = ',deltar
...
************************************************************************
      ...
      subroutine EPRC_INIT_ALPOS(LP1,LP2,LP3,LP4,LP5,
     $     LP6,LP7,LP8,LP9,LP10,
     $     LP11,LP12,LP13,LP14,LP15,
     $     LP16,LP17,LP18,LP19,LP20,
     $     doprint)
...
C...PURELY WEAK CONTRIBUTIONS
      ...
      if (doprint) write(6,*) 'end of EPRC_INIT_ALPOS dr = ',deltar
...
************************************************************************
      ...
      subroutine GETPAR_ALPOS(q2_in,DR_o,SWEFF2_o,AKAPPA_o,
     *                        SW2_o,MW_o,MZ_o)
...
c     call SETFFQ(T)
      ...
      MW_o=MW
      MZ_o=MZ
c      WWidth_o=DIMAG(SIGMQW(MW2))/MW
c      ZWidth_o=DIMAG(SIGMQZ(MZ2))/MZ
...
************************************************************************
      ...
      SUBROUTINE SETPAR_DEFAULT(doprint)
...
C---DEFINE PARAMETERS OF THE ELECTROWEAK STANDARD MODEL
      ...
      cau=0.5d0
      cad=-0.5d0
      cvu=0.2028037d0   !(1D0-8D0*SW2/3D0)/2d0
      cvd=-0.35140185d0 !(4D0*SW2/3D0-1D0)/2d0 
...
************************************************************************
      ...
      SUBROUTINE SETPAR_ALPOS(me_i,mmu_i,mtau_i,
     $     mu_i,md_i,ms_i,mc_i,mb_i,mt_i,
     $     mZ_i,mW_i,mH_i,alpha_i,gf_i,
     $     cau_i,cvu_i,cad_i,cvd_i,
     $     doprint)
...
C      PRINT *,PI,ME,MMY,MTAU,MU,MD,MS,MC,MB,MT,MZ,MW,MH,ALPHA,GF,CONV
...
c      SXNORM=PI*ALPHA*ALPHA/2D0*3.8938D5
c      SX1NRM=ALPHA*ALPHA*ALPHA/16D0/PI*3.8938D5
      SXNORM=PI*ALPHA*ALPHA/2D0*CONV
      SX1NRM=ALPHA*ALPHA*ALPHA/16D0/PI*CONV
...
C...Running fine structure constant using Burkhard's parametrization
      ...
      COMPLEX*16 HSSRGG,F
      REAL*8 PI,ALPHA,ALP1PI,ALP2PI,ALP4PI,E,GF,SXNORM,SX1NRM,CONV
      COMMON /KONST/  PI,ALPHA,ALP1PI,ALP2PI,ALP4PI,E,GF,SXNORM,SX1NRM
     *              ,CONV
...
c      IMPLICIT REAL*8(A-H,M,O-Z)
      COMMON /GSW/    SW,CW,SW2,CW2
     *              ,YMW,YMZ,YMH,YME,YMYMY,YMTAU,YMU,YMD,YMS,YMC,YMB,YMT
     *              ,YMW2,YMZ2,YMH2,YME2,YMYMY2,YMTAU2,YMU2,YMD2,YMS2
     *              ,YMC2,YMB2,YMT2
      COMMON /PARLIS/ LPAR(20),LPARIN(12),IPART
